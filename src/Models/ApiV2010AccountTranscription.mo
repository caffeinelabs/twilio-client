...
import { type TranscriptionEnumStatus; JSON = TranscriptionEnumStatus } "./TranscriptionEnumStatus";

// ApiV2010AccountTranscription.mo

module {
    // User-facing type: what application code uses
    public type ApiV2010AccountTranscription = {
    ...
    public module JSON {
        ...
        // Named "JSON" to avoid shadowing the outer ApiV2010AccountTranscription type
        public type JSON = {
            account_sid : ?Text;
            api_version : ?Text;
            date_created : ?Text;
            ...
            recording_sid : ?Text;
            sid : ?Text;
            status : ?TranscriptionEnumStatus.JSON;
            transcription_text : ?Text;
            type_ : ?Text;
            uri : ?Text;
        };
        ...
        public func toJSON(value : ApiV2010AccountTranscription) : JSON = { value with
            status = do ? { TranscriptionEnumStatus.toJSON(value.status!) };
        };

        // Convert JSON-facing Motoko type to User-facing type
        public func fromJSON(json : JSON) : ?ApiV2010AccountTranscription {
            ?{ json with
                status = do ? { TranscriptionEnumStatus.fromJSON(json.status!)! };
            }
        };
    }
}
